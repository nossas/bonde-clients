...
describe('client/mobilizations/widgets/__plugins__/pressure/components/__pressure__', () => {
  ...
  it('should color with main_color, h2, a, PressureForm, PressureCount', () => {
    ...
    expect(
      wrapper.find('InjectIntl(PressureForm)').props().buttonColor
    ).to.equal('#fff');
  ...
  it('should render h2 with title_text', () => {
    ...
    wrapper.setProps({
      widget: { id: 1, settings: { title_text: titleText } },
    });
  ...
  it('should render PressureForm with pressure_subject and pressure_body', () => {
    ...
    expect(wrapper.find('InjectIntl(PressureForm)').props().subject).to.equal(
      pressureSubject
    );
    expect(wrapper.find('InjectIntl(PressureForm)').props().body).to.equal(
      pressureBody
    );
  });
  ...
  it('should render PressureForm with button_text', () => {
    ...
    wrapper.setProps({
      widget: { id: 1, settings: { button_text: buttonText } },
    });

    expect(
      wrapper.find('InjectIntl(PressureForm)').props().buttonText
    ).to.equal(buttonText);
  });

  it('should hide PressureCount when show_counter is "false"', () => {
    wrapper.setProps({
      widget: { id: 1, settings: { show_counter: 'false' } },
    });
  ...
  it('should render targets received by settings with type string', () => {
    ...
    expect(wrapper.find('TargetList').props().targets).to.deep.equal([
    ...
    wrapper.setProps({
      widget: {
        ...
        settings: {
          targets:
            'Igor Santos <igor@nossascidades.org>;Lucas Pirola <pirola@nossascidades.org>',
        },
      },
    });
    expect(wrapper.find('TargetList').props().targets).to.deep.equal([
