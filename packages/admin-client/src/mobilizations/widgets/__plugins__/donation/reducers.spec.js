...
import reducers, {
  initialState,
} from '../../../../mobilizations/widgets/__plugins__/donation/reducers';
...
describe('client/mobilizations/widgets/__plugins__/donation/reducers', () => {
  ...
  it('should change saving state to false and error state with message when failed', () => {
    ...
    const failurePayload = {
      error: 'Donation widget transaction create request error message!',
    };
    const action = createAction(
      t.ASYNC_DONATION_TRANSACTION_CREATE_FAILURE,
      failurePayload
    );
    const nextState = reducers(currentInitialState, action);
