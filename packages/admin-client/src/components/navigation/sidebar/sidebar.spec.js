    ...
    describe('when is not editing a mobilization', () => {
      ...
      describe('community settings navbar items', () => {
        describe('- mobilizações', () => {
          ...
          it('should render with its text properly', () => {
            expect(
              wrapper.find('SidenavListItem').at(itemIndex).props().text.props
            ).to.be.deep.equal(
              (
                <FormattedMessage
          ...
          it('should render with its icon properly', () => {
            const icon = 'list';
            expect(
              wrapper.find('SidenavListItem').at(itemIndex).props().icon
            ).to.be.equal(icon);
          });
          it('should render with its href properly', () => {
            const href = paths.mobilizations();
            expect(
              wrapper.find('SidenavListItem').at(itemIndex).props().href
            ).to.be.equal(href);
          });
        });
