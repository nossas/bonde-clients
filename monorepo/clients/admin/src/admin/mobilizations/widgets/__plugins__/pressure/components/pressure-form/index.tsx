...
class PressureForm extends React.Component {
  constructor(properties) {
    ...
    this.state = {
      ...
      pressureType: pressureHelper.getType(properties.targetList) || undefined,
      callManagement: undefined
    }
  }

  componentWillReceiveProps(properties) {
    if (!this.props.callTransition && properties.targetList && properties.targetList.length > 0) {
      this.setState({
        callManagement: properties.targetList.map(target => ({
          ...parseTarget(target),
          attempts: 0
        }))
      })
    ...
    if (
      properties.callTransition &&
      properties.targetList &&
      properties.targetList.length > 0 &&
      JSON.stringify(this.props.callTransition) !== JSON.stringify(properties.callTransition)
    ) {
      this.setState({
        callManagement: this.state.callManagement.map((target, index) => {
          const isCallToCurrentTarget = target.value === properties.callTransition.twilioCallTo
          const transition = isCallToCurrentTarget ? properties.callTransition : {}
          const { twilioCallTransitionStatus: status } = transition
          const isFailStatus = ['busy', 'failed', 'no-answer'].includes(status)

          let { attempts } = this.state.callManagement[index]
  ...
  validate() {
    ...
    if (this.state.pressureType === pressureHelper.PRESSURE_TYPE_PHONE) {
      ...
      if (!phone) {
        ...
        errors.phone = [11, 12].includes(phoneE164.length) ? intl.formatMessage({
