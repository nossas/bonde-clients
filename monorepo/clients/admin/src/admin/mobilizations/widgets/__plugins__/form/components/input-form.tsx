...
export class InputForm extends React.Component {
  ...
  componentWillReceiveProps(nextProperties) {
    const { field } = this.props
    if (this.state.loading && field !== nextProperties.field) {
  ...
  handleOverlayClick(event) {
    ...
    if (!dirty || (dirty && window.confirm(this.props.intl.formatMessage({
            ...
            <div className='table col-12 mb2'>
              ...
              <div className='col-9 table-cell'>
                <select
                  ...
                  {Object.keys(this.fieldTypes).map((index, indexI) => (
                    <option key={indexI} value={index}>{this.props.intl.formatMessage(this.fieldTypes[index])}</option>
                  ))}
                </select>
              </div>
            </div>
