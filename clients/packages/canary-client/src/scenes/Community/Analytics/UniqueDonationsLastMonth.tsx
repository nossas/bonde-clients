...
const totalUniqueDonationsAmountLastMonth = gql`
  query ($communityId: Int!) {
    count: totalSumUniqDonationsFromCommunityInterval(
      comId: $communityId
      status: "paid"
...
const SubscriptionDonationsLastMonth = ({ communityId, children }: Props) => {
  const { data, loading, error } = useQuery(totalUniqueDonationsAmountLastMonth, { variables: { communityId } });
