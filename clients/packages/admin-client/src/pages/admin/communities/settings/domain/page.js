...
class Page extends Component {
  ...
  toggleDNSRecords(dnsHostedZone) {
    if (dnsHostedZone !== this.state.dnsHostedZone) {
      this.props.fetchDNSRecords(dnsHostedZone).then((dnsRecords) => {
  ...
  confirmDeleteSubdomain() {
    ...
    if (deleteDNSRecord && this.state.deletedDNSRecord) {
      ...
      deleteDNSRecord(this.state.deletedDNSRecord).then((dnsRecord) => {
        this.setState({
          dnsRecords: this.state.dnsRecords.filter(
            (d) => d.id !== dnsRecord.id
          ),
        });
      });
  ...
  dnsHostedZoneMenu(dnsHostedZone) {
    ...
    if (!dnsHostedZone.ns_ok) {
      items.splice(0, 0, {
        ...
        onClick: () => {
          checkHostedZone(dnsHostedZone).then((resp) => {
      ...
      <div className="domain-page">
        ...
        <div className="dns-hosted-zones">
          ...
          <ButtonPreview
            ...
            onClick={() =>
              this.props.history.push(Paths.communityDomainCreate())
            }
          />
                  ...
                  onSubmit={(values) => {
                    const name =
                      (values.name && values.name.length > 0
                        ? values.name + '.'
                        : '') + this.state.dnsHostedZone.domain_name;
                    const record_type = values.record_type
                      ? values.record_type
                      : 'A';
                    return createDNSRecord({
                      ...
                      this.setState({
                        dnsRecords: [...this.state.dnsRecords, dnsRecord],
                        showSubdomainForm: false,
                      });
