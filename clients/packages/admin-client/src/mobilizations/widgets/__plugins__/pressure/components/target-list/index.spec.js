...
describe('client/mobilizations/widgets/__plugins__/pressure/components/target-list', () => {
  ...
  beforeEach(() => {
    wrapper = shallow(<TargetList onSelect={() => {}} />);
  });
  ...
  describe('email pressure', () => {
    ...
    it('should render target parsed to Name <user@host.com>', () => {
      ...
      expect(wrapper.find('.target-item span').at(0).text()).to.equal(
        'Igor Santos'
      );
      expect(wrapper.find('.target-item span').at(1).text()).to.equal(
  ...
  describe('selectable', () => {
    it('should render the phone pressure label text properly', () => {
      ...
      expect(
        wrapper.find('.target-list-label FormattedMessage').at(0).props()
          .defaultMessage
    ...
    it('should render a checkbox on each target item', () => {
      ...
      expect(
        wrapper.find('.target-item input[type="checkbox"]').length
      ).to.equal(2);
    });
  });
});
