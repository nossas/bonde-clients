...
const Tag = ({ value, onClick, onRemove }) => {
  ...
  return (
    <span className="bg-white mr1 mt1 p1" style={{ cursor: clickable ? "pointer" : null }}>
      <span className={(removable ? "mr1" : null)} onClick={(clickable ? () => onClick(value) : null)}>{value}</span>
